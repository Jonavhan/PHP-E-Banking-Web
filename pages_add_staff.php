...
if (isset($_POST['import_data'])) {
    ...
    try {
        ...
        for ($row = $startingRow; $row <= $worksheet->getHighestRow(); $row++) {
            ...
            $staff_number = $worksheet->getCell('B' . $row)->getValue();
            $phone = $worksheet->getCell('C' . $row)->getValue();
            $email = $worksheet->getCell('D' . $row)->getValue();
            $password = sha1(md5($worksheet->getCell('E' . $row)->getValue()));
            $sex = $worksheet->getCell('F' . $row)->getValue();

            $query = "INSERT INTO ib_staff (name, staff_number, phone, email, password, sex) VALUES (?, ?, ?, ?, ?, ?)";
            $stmt = $conn->prepare($query);
            $stmt->bind_param('ssssss', $name, $staff_number, $phone, $email, $password, $sex);
            $stmt->execute();
        }
